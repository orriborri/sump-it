...
interface GrinderPreviewProps {
  formData: GrinderFormData
  stepsPerUnit: number
}

interface GrinderPreviewSliderProps {
  formData: GrinderFormData
}
  ...
  <Card variant="outlined" sx={{ bgcolor: 'grey.50' }}>
    <CardContent>
      ...
      <Stack spacing={2}>
        ...
        {/* Preview Slider */}
        <GrinderPreviewSlider formData={formData} />
      </Stack>
    </CardContent>
  </Card>
)

interface GrinderTypeSelectProps {
  value: string
...
interface GrinderRangeFieldsProps {
  formData: GrinderFormData
...
const GrinderRangeFields: React.FC<GrinderRangeFieldsProps> = ({ formData, onMinChange, onMaxChange }) => (
  <Stack direction="row" spacing={2}>
    ...
    <TextField
      ...
      type="number"
      value={formData.max_setting}
      onChange={e => onMaxChange(parseInt(e.target.value) || 40)}
...
interface GrinderFormActionsProps {
...
/** Full grinder configuration form with range, step size, type, and live preview. */
