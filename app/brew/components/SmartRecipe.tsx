...
export const SmartRecipe: React.FC<SmartRecipeProps> = ({
  ...
  useEffect(() => {
    if (selectedBrewId && previousBrews.length > 0) {
      const selectedBrew = previousBrews.find(brew => brew.id === selectedBrewId);
      if (selectedBrew) {
  ...
  }, [formData.water, formData.dose, formData.grind, formData.ratio, selectedBrewId, previousBrews]);
  ...
  return (
    <Box>
      ...
      {(formData.water || formData.dose || formData.grind || formData.ratio) && (
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ xs: 'flex-start', sm: 'center' }}>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap">
              <Chip 
                ...
                size="small"
              />
              {recommendation.basedOnBrews && recommendation.basedOnBrews > 0 && (
                <Chip 
      ...
      {loaded && previousBrews.length === 0 ? (
        <Box>
          ...
          <Typography variant="body2">
            This is your first time brewing with this combination of beans, method, and grinder.
            {recommendation && " We've provided recommended parameters above to get you started."}
          </Typography>
        </Box>
      ) : (
        <Box>
                ...
                <Grid item xs={12} sm={6} md={4} key={brew.id}>
                  <Card
                    sx={{
                      ...
                      border: isSelected ? '3px solid' : hasGoodRating ? '2px solid' : '1px solid',
                      borderColor: isSelected ? 'primary.main' : hasGoodRating ? 'success.main' : 'divider',
